...
class DQN(nn.Module):
    def __init__(self, input_shape: Tuple[int, int, int], n_actions: int):
        ...
        self.fc1 = nn.Linear(
            conv_out_size + 4, 512
        )  # +4 pour le statut des joueurs et le joueur actuel
    ...
    def sample(
        ...
        state = {
            ...
            "player_status": torch.FloatTensor(
                np.array([s["player_status"] for s in state])
            ),
            "current_player": torch.FloatTensor(
                np.array([s["current_player"] for s in state])
            ),
        }
        ...
        next_state = {
            ...
            "player_status": torch.FloatTensor(
                np.array([s["player_status"] for s in next_state])
            ),
            "current_player": torch.FloatTensor(
                np.array([s["current_player"] for s in next_state])
            ),
        }
